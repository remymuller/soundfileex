"""
    tests made for pytest
"""

def test_instrument_chunk():
    import numpy as np
    import math 
    ...
    with sfex.SoundFileEx("test.wav", "r") as snd:
        ...
        assert chunk.basenote == note
        #assert chunk.detune == detune # this one fails for now :(
        assert len(chunk.loops) == 1
        assert chunk.loops[0][0] == loop_start
        assert chunk.loops[0][1] == loop_end
    os.remove("test.wav")
